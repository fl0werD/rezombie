...
namespace rz
{
    ...
    inline auto TE_BSPDecal(Vector& position, int textureIndex, int entityIndex, int modelIndex) -> void {
        ...
        if (entityIndex != 0) {
            WriteShort(modelIndex); // index of model of above entity only included if previous short is non-zero (not the world)
        }
    }
    ...
    TE_Bubbles(
        ...
        WriteShort(modelIndex);    // models index
        WriteByte(count);          // count
